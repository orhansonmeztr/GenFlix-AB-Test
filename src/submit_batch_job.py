...
from config import GOOGLE_API_KEY, GEMINI_MODEL, PERSONA_CONFIGS
...
def prepare_and_submit_batch():
    """
    Prepares a JSONL file with generation requests for all personas and submits it
    ...
    print("--- Step 1: Preparing Batch Requests for ALL Personas with Custom Temperatures ---")
    ...
    with open(REQUESTS_FILE_PATH, "w") as f:
        for _, row in tqdm(sampled_df.iterrows(), total=len(sampled_df), desc="Preparing JSONL"):
            # Artık yeni konfigürasyon nesnesi üzerinde döngü kuruyoruz
            for persona_key, persona_config in PERSONA_CONFIGS.items():
                ...
                request_key = f"{row['show_id']}_{persona_key}"
    ...
    total_requests = len(sampled_df) * len(PERSONA_CONFIGS)
    print(f"Successfully created request file with {total_requests} total requests.")
    ...
    upload_display_name = f"genflix_requests_all_personas_{timestamp}"
    ...
    job_display_name = f"genflix_batch_all_personas_{timestamp}"
